...
final class StoppableCollectionHeaderView: UICollectionReusableView {
    ...
    override init(frame: CGRect) {
        ...
        self.addSubview(stoppableContainerView)
        ...
        let constraints: [NSLayoutConstraint] = [
            stoppableContainerView.leadingAnchor.constraint(equalTo: self.leadingAnchor),
            self.trailingAnchor.constraint(equalTo: stoppableContainerView.trailingAnchor),
            self.bottomAnchor.constraint(equalTo: stoppableContainerView.bottomAnchor),
            dmmyHeightConst
        ]
    ...
    func configure(stoppableView: UIView, viewController: UIViewController) {
        if !self.stoppableContainerView.subviews.contains(where: { $0 == stoppableView }) {
            print("\(#function) add")
            viewController.view.addSubview(stoppableView)
            stoppableView.translatesAutoresizingMaskIntoConstraints = false

            let yConst = stoppableView.centerYAnchor.constraint(equalTo: self.stoppableContainerView.centerYAnchor)
