...
final class StoppableHeaderPageCollectionViewController: UIViewController {
    ...
    private var viewControllers: [UIViewController] = {
        [
            UIStoryboard(name: "DummyCollection", bundle: nil).instantiateInitialViewController()!,
            UIStoryboard(name: "DummyCollection", bundle: nil).instantiateInitialViewController()!,
            UIStoryboard(name: "DummyCollection", bundle: nil).instantiateInitialViewController()!,
        ]
    }()
...
private extension StoppableHeaderPageCollectionViewController {
    ...
    func setupHeaderView() {
        ...
        let headerViewTopAnchor = headerView.topAnchor.constraint(equalTo: self.view.safeAreaLayoutGuide.topAnchor)
        headerViewTopAnchor.priority = .init(rawValue: 999)
        let headerViewConstraints = [
            headerViewTopAnchor,
            headerView.centerXAnchor.constraint(equalTo: self.view.centerXAnchor),
            headerStopView.topAnchor.constraint(greaterThanOrEqualTo: self.view.safeAreaLayoutGuide.topAnchor),
